        ...
        impl crate::gameobjects::Collides<> for $structname {
            ...
            fn collides_with(&self, other: &(dyn crate::gameobjects::Collides)) -> Option<crate::gameobjects::CollisionInfo> {
                ...
                if self.collider_type == crate::gameobjects::ColliderType::Convex && other.get_collider_type() == crate::gameobjects::ColliderType::Convex { 
                    ...
                    let mut support = crate::engine::gameobjects::collisions::support(&verts1, &verts2, direction); // support is farthest vertex along direction
                    simplex.push_front(support);
                    direction = -support;
                    //println!("BEGINNING TEST");
                    loop {
                        support = crate::engine::gameobjects::collisions::support(&verts1, &verts2, direction);
                        ...
                        simplex.push_front(support);
                        if crate::engine::gameobjects::collisions::next_simplex(&mut simplex, &mut direction) {
                            break;
                        }
                    }
                    ...
                    let (mut normals, mut min_face) = crate::engine::gameobjects::collisions::get_face_normals(&polyhedron, &faces);
                    ...
                    while min_distance == f32::MAX {
                        ...
                        let support2 = crate::engine::gameobjects::collisions::support(&verts1, &verts2, min_normal);
                        ...
                        if (s_distance - min_distance).abs() > 0.001 {
                            ...
                            while i < normals.len() {
                                ...
                                if normals[i].xyz().dot(&support2) < 0.0 {
                                    ...
                                    crate::engine::gameobjects::collisions::add_if_unique_edge(&mut unique_edges, &faces, f, f + 1);
                                    crate::engine::gameobjects::collisions::add_if_unique_edge(&mut unique_edges, &faces, f + 1, f + 2);
                                    crate::engine::gameobjects::collisions::add_if_unique_edge(&mut unique_edges, &faces, f + 2, f);
                                    println!("b.3");
                            ...
                            let (new_normals, new_min_face) = crate::engine::gameobjects::collisions::get_face_normals(&polyhedron, &new_faces);
                ...
                // sphere to box
                    ...
                    let sphere_pos: glm::Vec3 = crate::engine::transform::vec3_from_i64vec3(&(sphere.transform().pos() - cube.transform().pos()));
                    ...
                    let min_distance_squared = (sphere.transform().scl().x/2.0).powf(2.0) as f64 * (crate::engine::transform::UNITS_PER_METER.pow(2)) as f64; // didn't want to use doubles here but FP error, if can fix yay
                    let v = crate::engine::transform::i64vec3_from_vec3(&transformed_sphere_pos.xyz()) - crate::engine::transform::i64vec3_from_vec3(&closest);
                    //println!("Vec = {:?}", v);
                    ...
                    if min_distance_squared as i64 >= distance_squared {
                        ...
                        let hitpos = crate::engine::transform::i64vec3_from_vec3(&(mat2 * vec4(closest.x, closest.y, closest.z, 1.0)).xyz()) + cube.transform().pos();
                        println!("Extents were {:?} so pos is {:?}", extents, hitpos);
                        let info = crate::engine::gameobjects::collisions::CollisionInfo {
                            normal: (mat2 * glm::vec4(normal.x, normal.y, normal.z, 1.0)).xyz(),
                            collision_points: vec![(hitpos, (distance_squared as f64).sqrt() as i64 - ((sphere.transform().scl().x/2.0) as f64 * crate::engine::transform::UNITS_PER_METER as f64) as i64)]
                        };
                        return Some(info);
                    }
                    else {
                ...
                // sphere to sphere
                else if self.collider_type == crate::engine::gameobjects::collisions::ColliderType::Sphere && other.get_collider_type() == crate::engine::gameobjects::collisions::ColliderType::Sphere { 
                    let min_distance_squared = (self.transform.scl().x/2.0 + other.transform().scl().x/2.0).powf(2.0) as i64 * (crate::engine::transform::UNITS_PER_METER).pow(2);
                    ...
                    if min_distance_squared >= distance_squared {
                        let info = crate::engine::gameobjects::collisions::CollisionInfo {
                            normal: crate::engine::transform::vec3_from_i64vec3(&v),
                            collision_points: vec![(crate::engine::transform::i64vec3_from_vec3(&(crate::engine::transform::vec3_from_i64vec3(&v) * ((self.transform.scl().x/2.0)))) + self.transform.pos(), (distance_squared as f64).sqrt() as i64 - (self.transform.scl().x/2.0 + other.transform().scl().x/2.0) as i64 * (crate::engine::transform::UNITS_PER_METER))]
                        };
                        return Some(info)
                    }
                    else {
